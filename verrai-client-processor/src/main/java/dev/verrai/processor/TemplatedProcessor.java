...
public class TemplatedProcessor extends AbstractProcessor {
        ...
        for (javax.lang.model.element.Element element : roundEnv.getElementsAnnotatedWith(Templated.class)) {
            ...
            try {
                ...
                processingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR,
                        "Error processing @Templated: " + e.getMessage(), element);
                e.printStackTrace();
            }
        }
        return true;
    ...
    private void processType(TypeElement typeElement) throws IOException {
        ...
        for (VariableElement field : fields) {
            DataField dataField = field.getAnnotation(DataField.class);
            RestrictedAccess restricted = field.getAnnotation(RestrictedAccess.class);
            if (dataField != null && restricted != null) {
        ...
        for (ExecutableElement method : ElementFilter.methodsIn(typeElement.getEnclosedElements())) {
            ...
            if (eventHandler != null) {
                ...
                if (isHtmlElement) {
                    bindMethod.addCode("target.$L.addEventListener($S, e -> target.$L(", targetField, eventType,
                            method.getSimpleName());
                } else if (isWidget) {
                    bindMethod.addCode(
                            "((dev.verrai.api.IsWidget)target.$L).getElement().addEventListener($S, e -> target.$L(",
                            targetField, eventType, method.getSimpleName());
                } else {
                    ...
                    processingEnv.getMessager().printMessage(Diagnostic.Kind.WARNING,
                            "Field '" + targetField + "' is not HTMLElement or IsWidget. Event binding may fail.",
                            method);
                    // Try anyway assuming IsWidget cast might work or just access field.element
                    bindMethod.addCode(
                            "if (target.$L instanceof dev.verrai.api.IsWidget) ((dev.verrai.api.IsWidget)target.$L).getElement().addEventListener($S, e -> target.$L(",
                            targetField, targetField, eventType, method.getSimpleName());
                }
                ...
                if (events.length > 0) {
                    ...
                    if (isWidget) {
                        ...
                        for (String event : events) {
                            bindMethod.addCode(
                                    "((dev.verrai.api.IsWidget)target.$L).getElement().addEventListener($S, e -> ((dev.verrai.api.IsWidget)target.$L).onBrowserEvent(e));\n",
                                    field.getSimpleName(), event, field.getSimpleName());
                        }
                        bindMethod.endControlFlow();
                    } else {
                        processingEnv.getMessager().printMessage(Diagnostic.Kind.WARNING,
                                "@SinkNative is only supported on fields implementing IsWidget. Field '"
                                        + field.getSimpleName() + "' is ignored.",
                                field);
                    }
                }
            }
        ...
        if (modelField != null) {
                ...
                if (bound != null) {
                    ...
                    for (int i = 0; i < segments.length; i++) {
                        ...
                        for (ExecutableElement method : ElementFilter
                                .methodsIn(processingEnv.getElementUtils().getAllMembers(currentType))) {
                        ...
                        if (getterMethod == null) {
                            processingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR,
                                    "Property '" + segment + "' not found in " + currentType.getSimpleName()
                                            + ". Missing getter.",
                                    field);
                            valid = false;
                            break;
                        ...
                        if (!getterMethod.getModifiers().contains(javax.lang.model.element.Modifier.PUBLIC)) {
                            processingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR,
                                    "Getter for '" + segment + "' in " + currentType.getSimpleName()
                                            + " is not public.",
                                    field);
                            valid = false;
                            break;
                        ...
                        if (isLast) {
                            ...
                            if (setterMethod == null) {
                                processingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR,
                                        "Property '" + segment + "' in " + currentType.getSimpleName()
                                                + " is read-only. Missing setter.",
                                        field);
                                valid = false;
                            } else if (!setterMethod.getModifiers()
                                    .contains(javax.lang.model.element.Modifier.PUBLIC)) {
                                processingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR,
                                        "Setter for '" + segment + "' in " + currentType.getSimpleName()
                                                + " is not public.",
                                        field);
                                valid = false;
                            } else {
                            ...
                            if (returnType.getKind() == javax.lang.model.type.TypeKind.DECLARED) {
                                currentType = (TypeElement) ((javax.lang.model.type.DeclaredType) returnType)
                                        .asElement();
                                getterChain.append(".").append(getterMethod.getSimpleName()).append("()");
                                checkChain.append(" && target.").append(modelField.getSimpleName()).append(getterChain)
                                        .append(" != null");
                            } else {
                                processingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR,
                                        "Property '" + segment + "' in " + currentType.getSimpleName()
                                                + " is not an object, cannot access nested property.",
                                        field);
                                valid = false;
                                break;
                            }
                    ...
                    if (valid) {
                        ...
                        if (widgetValueType != null
                                && !processingEnv.getTypeUtils().isAssignable(modelPropType, widgetValueType)
                                && !processingEnv.getTypeUtils().isAssignable(widgetValueType, modelPropType)) {
                            needsConversion = true;
                        }

                        // Null check: target.model != null && target.model.getProp() != null ...
                        bindMethod.beginControlFlow("if (target.$L != null" + checkChain + " && target.$L != null)",
                                modelField.getSimpleName(), field.getSimpleName());
                        ...
                        if (needsConversion) {
                            ...
                            // Converted Handler
                            String inputEventName_conv = getInputEventName(field);
                            bindMethod.addCode(
                                    "((dev.verrai.api.IsWidget)target.$L).getElement().addEventListener($S, e -> {\n",
                                    field.getSimpleName(), inputEventName_conv);

                            com.squareup.javapoet.TypeName modelTypeName = com.squareup.javapoet.TypeName
                                    .get(modelPropType);
                            com.squareup.javapoet.TypeName boxedModelTypeName = modelPropType.getKind().isPrimitive()
                                    ? modelTypeName.box()
                                    : modelTypeName;
                            ...
                            if (modelPropType.getKind().isPrimitive()) {
                                ...
                                bindMethod.addStatement("target.$L$L.$L(value)", modelField.getSimpleName(),
                                        getterChain, finalSetter);
                                bindMethod.endControlFlow();
                            } else {
                                bindMethod.addStatement("target.$L$L.$L(value)", modelField.getSimpleName(),
                                        getterChain, finalSetter);
                        ...
                        bindMethod.beginControlFlow("if (target.$L instanceof dev.verrai.api.binding.BindableProxy)",
                                modelField.getSimpleName());
                        bindMethod.addStatement(
                                "$T subscription = ((dev.verrai.api.binding.BindableProxy) target.$L).addPropertyChangeHandler((p, v) -> {\n"
                                        +
                                        "  if (p.equals($S)) {\n",
                        ...
                        if (needsConversion) {
                            ...
                            bindMethod.addStatement(
                                    "    target.$L.setValue(($T) converter_refresh.toWidget(target.$L$L.$L()))",
                                    field.getSimpleName(),
                                    com.squareup.javapoet.TypeName.get(widgetValueType),
                                    modelField.getSimpleName(), getterChain, finalGetter);
                            bindMethod.endControlFlow();
                        ...
                        bindMethod.addStatement(
                                "((dev.verrai.api.binding.BinderLifecycle) target).addBinding(subscription)");
                        bindMethod.endControlFlow();

                        bindMethod.endControlFlow();

                        // Nested Property Listener (for paths like "address.street")
                        if (segments.length > 1) {
                            String finalPropName = segments[segments.length - 1];
                            String nestedSubVar = "nestedSub_" + field.getSimpleName();
                            bindMethod.beginControlFlow(
                                    "if (target.$L$L instanceof dev.verrai.api.binding.BindableProxy)",
                                    modelField.getSimpleName(), getterChain);
                            bindMethod.addStatement(
                                    "$T $L = ((dev.verrai.api.binding.BindableProxy) target.$L$L).addPropertyChangeHandler((p, v) -> {\n"
                                            + "  if (p.equals($S)) {\n"
                                            + "    target.$L.setValue(target.$L$L.$L());\n"
                                            + "  }\n"
                                            + "})",
                                    dev.verrai.api.binding.Subscription.class, nestedSubVar,
                                    modelField.getSimpleName(), getterChain,
                                    finalPropName,
                                    field.getSimpleName(), modelField.getSimpleName(), getterChain, finalGetter);
                            bindMethod.beginControlFlow("if (target instanceof dev.verrai.api.binding.BinderLifecycle)");
                            bindMethod.addStatement(
                                    "((dev.verrai.api.binding.BinderLifecycle) target).addBinding($L)", nestedSubVar);
                            bindMethod.endControlFlow();
                            bindMethod.endControlFlow();
                        }

                        // Change Handler
                        String inputEventName = getInputEventName(field);
                        bindMethod.addCode(
                                "((dev.verrai.api.IsWidget)target.$L).getElement().addEventListener($S, e -> {\n",
            ...
            for (ExecutableElement method : ElementFilter
                    .methodsIn(processingEnv.getElementUtils().getAllMembers(modelType))) {
                String methodName = method.getSimpleName().toString();
                if (method.getParameters().isEmpty()
                    ...
                    if (methodName.startsWith("get") && methodName.length() > 3) {
                        ...
                        for (ExecutableElement m : ElementFilter
                                .methodsIn(processingEnv.getElementUtils().getAllMembers(modelType))) {
            ...
            if (!allProperties.isEmpty()) {
                processingEnv.getMessager().printMessage(Diagnostic.Kind.WARNING,
                        "The following properties in model " + modelType.getSimpleName() + " are not bound: "
                                + allProperties,
                        typeElement);
            }
        }
    ...
    private String readTemplate(TypeElement typeElement, String templateName) {
        ...
        try {
            ...
            try {
                FileObject resource = processingEnv.getFiler().getResource(StandardLocation.CLASS_PATH, packageName,
                        templateName);
                String content = resource.getCharContent(true).toString();
                templateCache.put(cacheKey, content);
                return content;
    ...
    private String getInputEventName(VariableElement field) {
        TypeElement htmlInputType = processingEnv.getElementUtils()
                .getTypeElement("org.teavm.jso.dom.html.HTMLInputElement");
        if (htmlInputType != null && processingEnv.getTypeUtils()
                .isAssignable(field.asType(), htmlInputType.asType())) {
            return "input";
        }
        return "change";
    }
