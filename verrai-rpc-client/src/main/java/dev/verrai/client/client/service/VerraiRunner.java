...
public class VerraiRunner {

    private static final Logger logger = LoggerFactory.getLogger(VerraiRunner.class);

    public interface Configurer {
        void configure(UnitRegistry registry);
    }

    public static <T extends VerraiApplication> void run(Class<T> appClass, Configurer configurer) {
        try {
            ...
            throw new RuntimeException("Failed to run VerraiApplication", e);
        }
    }
    ...
    private static <T> void checkDone(AtomicInteger remaining, T appInstance) {
        if (remaining.decrementAndGet() == 0) {
            ...
            if (appInstance instanceof VerraiApplication) {
                ((VerraiApplication) appInstance).onStart();
            }
        }
    }
        ...
        for (Class<?> iface : value.getClass().getInterfaces()) {
            ...
            if (implAnnotated || ifaceAnnotated) {
                ...
                try {
                    ...
                    dev.verrai.client.service.transport.ServiceDispatcher dispatcher = (dev.verrai.client.service.transport.ServiceDispatcher) dispatcherClass
